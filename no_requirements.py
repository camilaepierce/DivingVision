...
def train_epoch_pretrain_semantic(model, train_loader, optimizer, device, augmentation_level="moderate"):
    """Pretrain one epoch using semantic_contrastive_loss with same-label samples as positive pairs.
    
    Features are trained to be closer for samples with the same class label.
    Positive pairs are identified by matching labels in the batch (not augmentations).
    """
    model.train()
    total_loss = 0.0
    n = 0
    ...
    for videos, labels in tqdm(train_loader, desc="Semantic pretrain", leave=False):
        ...
        # Forward pass on raw videos (no augmentation)
        # Positive pairs are samples with the same label, not augmentations of the same sample
        _logits, feats = model(videos)

        # Compute semantic contrastive loss
        # semantic_contrastive_loss uses SIM_MATRIX[labels][:, labels] to identify positive pairs
        # Samples with the same label (e.g., both class 5) have SIM_MATRIX[5][5] = 1.0
